// =============================================
// form.js v8 — Global Máster IM Digital Business School
...
async function sendWebhook(data, leadId) {
  ...
  const params = new URLSearchParams({
    ...
    comentarios:  buildComentarios({ ...data, ...getPais() }),
...
function validate(fields) {
  ...
  if (!rgpd || !rgpd.checked) {
    errors.push('Debes aceptar la política de privacidad para continuar');
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Guardar UTMs en sessionStorage
  const utms = getUTMs();
